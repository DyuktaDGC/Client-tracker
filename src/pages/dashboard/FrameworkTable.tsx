...
const columns: Column<BreakdownRow>[] = [
  {
    ...
    header: 'Week',
    align: 'center',
    className: 'w-[3.5rem] sm:w-[4.5rem]',
    render: (row) =>
      row.week === null ? (
  ...
  },
  {
    ...
    render: (row) => (
      <div className="flex min-w-[10rem] items-baseline gap-1.5 sm:min-w-[14rem] sm:gap-2">
        ...
        {row.kind === 'framework' || row.month === null ? null : (
          <span className="hidden whitespace-nowrap text-[10px] font-bold uppercase tracking-[0.06em] text-ink-faint sm:inline">C{row.month}</span>
        )}
      </div>
    ),
  },
  ...
  },
  {
    ...
    header: '%',
    align: 'right',
    className: 'w-[4.5rem] sm:w-[5.5rem]',
  ...
  },
  {
    ...
    header: 'Score',
    align: 'right',
    className: 'w-[5.5rem] sm:w-[7.5rem]',
    render: (row) => (
